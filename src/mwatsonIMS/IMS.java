...
public class IMS {
	...
	public void createProduct() {
		...
		// do-while loop which allows the user to create as many products as
		// they need
		do {
		...
		try {
			...
			for (Product p : allproducts) {
				...
				writer.write(String.format("%20s %20s %20s \r\n",
						String.valueOf(p.getProductID()), p.getProductName(),
		...
		try {
			...
			for (Product p : allproducts) {
				...
				if (p.getProductID() == id) {
					System.out
							.println("What do you want to change the stock to?");
					qty = scan.nextInt();
					p.setProductQty(qty);
					// System.out.println("New quantity for Product ID: " + id +
					// " Quantity: " + qty);
				}
